...
public class PrimeiraAulaTest {
	...
	public void testSomaResultadoArgumentoNulo() {
		...
		double[] valores = null;	                     	// ver a última referência a valores (null)
		double esperado = 0.0;
		double resultado = 0.0;
		int comparacao = 0;
		
		esperado = Double.NaN;
		resultado = this.obj.somaTotal(valores);            // primeira coisa a procurar (chamada do método). Segundo, identificar argumento (valores)
		comparacao = Double.compare(resultado, esperado);   // compara dois doubles
		Assert.assertEquals(
